...
BinaryTreeNode<int>* buildTreehelper(int *postorder, int poststart,int postend, int *inorder, int instart,int inend){
	...
	int postls=poststart;
	int postle=inle-inls+postls;
	int inrs=rootindex+1;
	int inre=inend;
	int postrs=postle+1;
	int postre=postend-1;
	BinaryTreeNode<int>* root=new BinaryTreeNode<int>(rootdata);
	root->left=buildTreehelper(postorder, postls,postle, inorder, inls,inle);
	root->right= buildTreehelper(postorder, postrs,postre, inorder, inrs,inre);
	return root;
